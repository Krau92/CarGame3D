...
namespace ArcadeVP
{
    public class ModArcadeVehicleController : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (grounded())
            {
                ...
                if (movementMode == MovementMode.AngularVelocity)
                {
                    ...
                    {
                        rb.angularVelocity = Vector3.Lerp(rb.angularVelocity, carBody.transform.right * accelerationInput * MaxSpeed * currentGroundTypeMultiplier / radius, accelaration * Time.deltaTime);
                    }
                ...
                else if (movementMode == MovementMode.Velocity)
                {
                    if (Mathf.Abs(accelerationInput) > 0.1f && !brakeInput && !kartLike)
                    {
                        rb.linearVelocity = Vector3.Lerp(rb.linearVelocity, carBody.transform.forward * accelerationInput * MaxSpeed * currentGroundTypeMultiplier, accelaration / 10 * Time.deltaTime);
                    }
                    else if (Mathf.Abs(accelerationInput) > 0.1f && kartLike)
                    {
                        rb.linearVelocity = Vector3.Lerp(rb.linearVelocity, carBody.transform.forward * accelerationInput * MaxSpeed * currentGroundTypeMultiplier, accelaration / 10 * Time.deltaTime);
